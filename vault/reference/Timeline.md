...
Response quality for v1 is whatever the leading config earns in pairwise eval before freeze, but will naturally be constrained by the above. v2 and later (with a larger corpus, richer retrieval, tool access) will be under ongoing development after the v1 release, always passing through the same quality and safety eval loops (pending changes we decide on based on what we learn from the v1 rollout process)
